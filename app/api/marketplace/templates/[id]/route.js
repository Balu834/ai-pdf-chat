...
export async function GET(req, { params }) {
  try {
    ...
    console.error("[marketplace/templates/[id]/route.js GET]", err?.message ?? err);
...
export async function POST(req, { params }) {
  try {
    ...
    // Increment counter + log usage (non-blocking)
    sb.rpc("increment_install_count", { p_type: "template", p_id: params.id }).then(() => {});
    logUsage(sb, user.id, "template", params.id, "install");

    return NextResponse.json({ installed: true, workflow_id: wf.id });
  } catch (err) {
    console.error("[marketplace/templates/[id]/route.js POST]", err?.message ?? err);
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
