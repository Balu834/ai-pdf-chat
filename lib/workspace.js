...
/**
 * Write a row to workspace_audit_log (best-effort, never throws).
 * Uses the admin client so it bypasses RLS — all writes are server-side only.
 */
export async function logAudit(workspaceId, actorId, action, targetId = null, meta = {}) {
  try {
    await getAdminClient().from("workspace_audit_log").insert({
