...
export async function getCredits(userId) {
  try {
    ...
    console.warn("[credits] getCredits failed (non-fatal):", err.message);
...
export async function getRecentTransactions(userId, limit = 10) {
  try {
    ...
    console.warn("[credits] getRecentTransactions failed (non-fatal):", err.message);
    return [];
  }
}
...
export async function getUsageSummary(userId) {
  try {
    ...
    console.warn("[credits] getUsageSummary failed (non-fatal):", err.message);
    return { total_tokens: 0, cost_usd: 0, credits_used: 0, questions: 0 };
  }
}
