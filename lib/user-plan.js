...
const USER_PLAN_SELECT =
  "plan, pro_expires_at, grace_until, subscription_status, is_trial, trial_start, trial_end, razorpay_subscription_id";
...
export async function getUserPlan(userId) {
  try {
    ...
    const { data: newRow, error: upsertErr } = await getAdminClient()
      .from("user_plans")
      .upsert(
...
export async function isProActive(userId) {
  try {
    ...
    if (data.plan !== "pro" && data.plan !== "team" && data.plan !== "premium") return false;
...
export async function upsertUserPlan(userId, fields) {
  try {
    const { error } = await getAdminClient().from("user_plans").upsert(
