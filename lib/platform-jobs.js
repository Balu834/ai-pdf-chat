...
export async function updateJob(jobId, patch) {
  const { error } = await admin().from("platform_jobs").update(patch).eq("id", jobId);
  if (error) throw error;
}

export async function getJob(jobId) {
  const { data } = await admin().from("platform_jobs").select("*").eq("id", jobId).single();
  return data;
}
...
export async function getJobWithLogs(jobId) {
  ...
  const [{ data: job }, { data: logs }] = await Promise.all([
    db.from("platform_jobs").select("*").eq("id", jobId).single(),
...
export async function addLog(jobId, level, message, data = null, step = 0) {
  await admin().from("job_logs").insert({ job_id: jobId, level, message, data, step });
}
