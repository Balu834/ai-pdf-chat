...
export async function checkPdfLimit(supabase, userId) {
  try {
    ...
    return { count, exceeded, remaining: Math.max(0, FREE_PLAN.maxPdfs - count), plan };
  } catch (err) {
...
export async function checkQuestionLimit(supabase, userId) {
  try {
    ...
    if (error) {
      ...
      return { count: 0, exceeded: false, remaining: FREE_PLAN.maxQuestions, plan };
    }

    const count = data?.total_questions ?? 0;
    const exceeded = count >= FREE_PLAN.maxQuestions;
    return { count, exceeded, remaining: Math.max(0, FREE_PLAN.maxQuestions - count), plan };
  } catch (err) {
