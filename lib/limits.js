...
export async function checkPdfLimit(supabase, userId) {
  try {
    ...
    if (error) {
      ...
      return { count: 0, exceeded: false, remaining: FREE_PLAN.maxPdfsPerDay, plan };
    }

    const safeCount = count ?? 0;
    const exceeded = safeCount >= FREE_PLAN.maxPdfsPerDay;
    return { count: safeCount, exceeded, remaining: Math.max(0, FREE_PLAN.maxPdfsPerDay - safeCount), plan };
  } catch (err) {
    console.warn("[limits] checkPdfLimit threw:", err.message);
    return { count: 0, exceeded: false, remaining: FREE_PLAN.maxPdfsPerDay, plan: "free" };
  }
}
